...
public class DraftListScreen extends JPanel {
	...
	private Client c;
	public DraftListScreen(Client c){
...
//		table.setSize(MailHome.screenSize.width, MailHome.screenSize.height);
	...
	//Table row click event handler
	 private void tableMouseClicked(java.awt.event.MouseEvent evt) {                                     
	       ...
	       mail = this.c.getDraft(mail);
	       
	       this.c.currentMail.setDraft(true);
	       System.out.println("printing in tablemouseclicked:draft: "+this.c.currentMail.getContent() + this.c.currentMail.getSubject()+ this.c.currentMail.isDraft()+this.c.currentMail.getToUser().getEmail());
	       mail.setDraft(true);
	       final ComposeScreen is = new ComposeScreen(this.c);
...
//	       jTextFieldAGE.setText(model.getValueAt(selectedRowIndex, 3).toString());
	...
	public void fillmodel(){
		...
		List<SubMail> mList = c.getDrafts(c.user);
		int i = 0;
